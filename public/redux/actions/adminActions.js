...
// Action to handle uploading attendance
export const uploadAttendance = (formData) => async (dispatch) => {
  try {
    const { data } = await api.uploadAttendance(formData);
    dispatch({ type: UPLOAD_ATTENDANCE, payload: data });
  } catch (error) {
    dispatch({ type: SET_ERRORS, payload: error.response.data || 'Something went wrong' });
  }
};
